...
// ================================================
// 4️⃣ MOBILE NAVIGATION (CON ANIMACIÓN X)
// ================================================
...
hamburger.addEventListener('click', () => {
    ...
    hamburger.classList.toggle('active'); // <--- ESTA LÍNEA HACE LA MAGIA DE LA "X"
...
navLinks.forEach(link => {
    link.addEventListener('click', () => {
        ...
        hamburger.classList.remove('active'); // <--- Y ESTA PARA QUE VUELVA A SER RAYITAS
...
// ================================================
// 9️⃣ FORMULARIO CON VALIDACIÓN SHAKE Y TOAST
// ================================================
...
contactForm.addEventListener('submit', (e) => {
    ...
    if (!service.value) {
        ...
        isValid = false;
        setTimeout(() => {
            service.closest('.form-group').classList.remove('error');
        }, 500);
    }
    ...
    const whatsappText =
        `👑 *Hola Sharlotte Styles* 👑%0A%0A` +
        `Soy *${encodeURIComponent(name.value.trim())}* y deseo agendar una cita.%0A` +
        `✨ Servicio: *${encodeURIComponent(service.value)}*%0A` +
        (message.value.trim() ? `📝 Nota: ${encodeURIComponent(message.value.trim())}%0A%0A` : '%0A') +
        `Quedo atenta/o. ✨`;
    ...
    setTimeout(() => {
        window.open(`https://wa.me/573042466814?text=${whatsappText}`, '_blank');
